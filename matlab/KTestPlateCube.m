...
nodes= [0, 0, 0; Lx, 0, 0; Lx, Ly, 0; 0, Ly, 0;
    0, 0, Lz; Lx, 0, Lz; 0, Ly, Lz];

% tria = struct('nodes', []);
tria = struct('nodes', [1, 2, 4; 2, 3, 4; 
    1, 2, 5; 2, 6, 5; 
    1, 4, 5; 4, 7, 5]);

beams = struct('nodes', []);
% beams = struct('nodes', [1,2; 1,4; 1, 5]);

mat = struct('E', Es, 'nu', nu);
prop.tria = struct('t', t);
...
fprintf ('the eigenvalues of the stiffnes matrix are : \n');
disp (sort(eig(Keps)).');
